...
from .runner_utils import Report
from ..utils import log


class Runner:
    def __init__(self, layer, module, loss_fn, opt, layer_map, options):
        self.trainer = Trainer(layer, module, loss_fn, layer_map, options)
        self.optimizer_helper = OptimizerHelper(opt, options)
        self.options = options
        self.steps = options["steps"]
        self.layer_map = layer_map

    def do_train(self, paddle_report, torch_report, example_inp):
        self.trainer.set_report(paddle_report, torch_report)
        self.trainer.train_step(example_inp)

    def do_check(self, paddle_report, torch_report):
        ret = Checker.check_forward_and_backward(torch_report, paddle_report, self.options)
        if ret == False:
            return ret

        weight_check, grad_check = Checker.weight_grad_check(
            self.trainer.layer, self.trainer.module, options=self.options, layer_map=self.layer_map
        )
    ...
    def run_normal(self, example_inp):
        for step_id in range(self.options["steps"]):
            ...
            self.do_train(paddle_report, torch_report, example_inp)
            self.do_optimizer()
